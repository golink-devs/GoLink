...
try_download_prebuilt() {
    ...
    if curl -fsL "$download_url" -o "$tmp_zip"; then
        ...
        unzip -j -o "$tmp_zip" "include/dave/array_view.h" -d "$INC_DIR"
        unzip -j -o "$tmp_zip" "include/dave/dave_interfaces.h" -d "$INC_DIR"
        unzip -j -o "$tmp_zip" "include/dave/logger.h" -d "$INC_DIR"
        unzip -j -o "$tmp_zip" "include/dave/version.h" -d "$INC_DIR"
